...
const fetchDataAndStore = async () => {
  try {
    ...
    let sortedBySellPrice = Object.values(data).sort(
      (a, b) => b.sell - a.sell
    );
    ...
    console.log(
      'Top 10 data by highest sell prices inserted successfully',
      top10Data
    );
  } catch (error) {
...
app.get('/api/tickers', async (req, res) => {
  try {
    const result = await pool.query(
      'SELECT name, last, buy, sell, volume, base_unit FROM ticker_data ORDER BY sell DESC LIMIT 10'
    );
